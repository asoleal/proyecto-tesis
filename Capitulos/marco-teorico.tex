...
Aunque el metano (CH\textsubscript{4}) es un gas de efecto invernadero con un potencial de calentamiento global significativamente mayor que el del dióxido de carbono (CO\textsubscript{2}), su generación en sistemas de bioconversión con BSF es generalmente baja debido al carácter predominantemente aeróbico del proceso. No obstante, se incluye su monitoreo como variable de control para detectar condiciones transitoriamente anaeróbicas —por ejemplo, por sobresaturación del sustrato o mala aireación— que podrían comprometer la eficiencia del proceso y aumentar emisiones no deseadas. Sin embargo, el foco técnico de esta tesis recae en el (CO\textsubscript{2}), ya que constituye el principal indicador cuantitativo y dinámico de la actividad biológica total (larval y microbiana) durante la bioconversión, presenta una señal más estable y continua para modelado en tiempo real, y permite una correlación directa con variables operativas como consumo de sustrato, biomasa generada y tasas respiratorias. Por estas razones, (CO\textsubscript{2})se adopta como la variable central para el desarrollo, calibración y validación del sistema híbrido propuesto.
